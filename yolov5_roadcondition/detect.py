...
def detect(save_img=False):
    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            cv2.putText(im0, '{} | inference: {:.4f}s | fps: {:.4f}'.format(opt.weights[0], t2-t1, 1/(time.time()-fps0)),(x+20, y+50),cv2.FONT_HERSHEY_SIMPLEX,1.5 ,(0,0,255),2)
    ...
    if torch.cuda.is_available():
        print_block('* GPU: {} - {}MB '.format(torch.cuda.get_device_properties(0).name, torch.cuda.get_device_properties(0).total_memory/1024**2))
    else:
